...
/**
 * 3.4.4 E-Posta Kanalı (Resend / SMTP Gateway) Implementasyonu.
 */
class EmailNotificationServiceImpl(
    ...
    override suspend fun sendNotification(payload: NotificationPayload, tenantId: String): Result<NotificationResult> {
        return runCatching {
            val params = buildJsonObject {
                ...
                put("p_title", payload.title ?: "TourOS E-Posta Bilgilendirmesi")
                put("p_content", payload.content)
                put("p_provider", "Resend/Email")
            }
    ...
    private fun generateFallback(payload: NotificationPayload): NotificationResult {
        return NotificationResult(
            id = "email-${(10000..99999).random()}",
            channel = NotificationChannel.EMAIL,
            recipient = payload.recipient,
            title = payload.title ?: "TourOS E-Posta Bilgilendirmesi",

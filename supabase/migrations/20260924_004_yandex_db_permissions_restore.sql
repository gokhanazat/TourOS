-- Yandex Cloud PostgreSQL: marketplace_products Yetkileri ve Fonksiyon Tanımları
GRANT ALL ON public.marketplace_products TO postgres, service_role;
...
SET sync_requested = false,
    ...
    synced_record_count = (SELECT COUNT(*) FROM public.marketplace_products WHERE product_type = 'PACKAGE_TOUR'),
    status_message = '🟢 CANLI (%100 Yenilendi • ' || (SELECT COUNT(*) FROM public.marketplace_products WHERE product_type = 'PACKAGE_TOUR') || ' Tur Aktif)'
